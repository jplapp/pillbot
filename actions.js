...
const MAX_PILL_TAKE_TIME = 12; // after X hours, give up for the day
...
module.exports = class {
    ...
    constructor(db, bot){
        this.db = db;
        this.bot = bot;
        this.MAX_PILL_TAKE_TIME = MAX_PILL_TAKE_TIME;
    ...
    checkIfShouldTakePill(user, lastTakenDate, now=undefined){
        let start = moment.unix(user.start_date);
        if(isNaN(now)){
            now = moment();
        }

        console.log('start', start.format(), 'now', now.format(), now.diff(start, 'hours'));
